$AppId = "<AppID>"
$client_secret = "<Client Secret>" | ConvertTo-SecureString -AsPlainText -Force
$TenantId = "<Tenant ID>"
...
do {
    $result = Invoke-WebRequest -Headers $AuthHeader1 -Uri $uri -SkipHeaderValidation -ErrorAction Stop
...
foreach ($team in $Teams) {
    ...
    $teamApps = Invoke-WebRequest -Headers $authHeader1 -Uri "https://graph.microsoft.com/v1.0/teams/$($Team.id)/installedApps?`$expand=teamsApp" -SkipHeaderValidation -ErrorAction Stop
    ...
    $TeamChannels = Invoke-WebRequest -Headers $AuthHeader1 -Uri "https://graph.microsoft.com/beta/Teams/$($Team.id)/channels" -SkipHeaderValidation -ErrorAction Stop
    $TeamChannels = ($TeamChannels.Content | ConvertFrom-Json).value
    
    foreach ($channel in $TeamChannels) {
        $tabs = Invoke-WebRequest -Headers $authHeader1 -Uri "https://graph.microsoft.com/beta/teams/$($Team.id)/channels/$($channel.id)/tabs?`$expand=teamsApp" -SkipHeaderValidation -ErrorAction Stop
...
$ReportApps | select * -ExcludeProperty Number | Export-Csv -Path "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_TeamsAppsReport.csv" -NoTypeInformation -Delimiter ";" 
$ReportTabs | select * -ExcludeProperty Number | Export-Csv -Path "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_TeamsTabsReport.csv" -NoTypeInformation -Delimiter ";"
